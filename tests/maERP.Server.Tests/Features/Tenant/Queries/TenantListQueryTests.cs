...
public class TenantListQueryTests : TenantIsolatedTestBase
{
    ...
    public async Task GetTenantList_WithSearchString_ShouldFilterTenants()
    {
        ...
        var response = await Client.GetAsync("/api/v1/Tenants?searchString=Test%20Tenant%202");
    ...
    public async Task GetTenantList_WithPagination_ShouldRespectPageSize()
    {
        ...
        var response = await Client.GetAsync("/api/v1/Tenants?pageNumber=0&pageSize=2");
