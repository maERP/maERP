...
/// <summary>
/// Model for invoice list page using MVUX pattern.
...
public partial record InvoiceListModel
{
    ...
    public IListFeed<InvoiceListDto> Invoices => Feed
        ...
        .SelectAsync(async (combined, ct) =>
        {
            ...
            var response = await _invoiceService.GetInvoicesAsync(parameters, ct);

            // Update pagination info
            await Pagination.UpdateAsync(_ => new InvoicePaginationInfo(
    ...
    /// </summary>
    public async Task ViewInvoice(InvoiceListDto invoice)
    {
        await _navigator.NavigateDataAsync(this, new InvoiceDetailData(invoice.Id));
    }
...
/// <summary>
/// Holds pagination state information for invoices.
/// </summary>
public record InvoicePaginationInfo
{
    private readonly IStringLocalizer? _localizer;
    ...
    /// <summary>
    /// Display text for total count info (e.g., "25 invoices").
    /// </summary>
    public string CountInfo
    {
        get
        {
            if (TotalCount == 1)
            {
                return _localizer?["Pagination.InvoicesSingular"] ?? "1 invoice";
            }

            var format = _localizer?["Pagination.InvoicesPlural"] ?? "{0} invoices";
...
/// <summary>
/// Navigation data for invoice detail page.
/// </summary>
public record InvoiceDetailData(Guid invoiceId);
