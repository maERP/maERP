...
/// <summary>
/// Model for warehouse list page using MVUX pattern.
...
public partial record WarehouseListModel
{
    ...
    /// </summary>
    public IState<WarehousePaginationInfo> Pagination => State<WarehousePaginationInfo>.Value(this, () => new WarehousePaginationInfo());

    /// <summary>
    /// Feed of warehouses from the API.
    /// Automatically refreshes when SearchQuery, CurrentPage, or SortOrder changes.
    /// </summary>
    public IListFeed<WarehouseListDto> Warehouses => Feed
        ...
        .SelectAsync(async (combined, ct) =>
        {
            ...
            await Pagination.UpdateAsync(_ => new WarehousePaginationInfo(
...
/// <summary>
/// Holds pagination state information for warehouses.
/// </summary>
public record WarehousePaginationInfo
{
    private readonly IStringLocalizer? _localizer;

    public WarehousePaginationInfo()
    {
    }
    ...
    /// <summary>
    /// Display text for total count info (e.g., "25 warehouses").
    /// </summary>
    public string CountInfo
    {
        get
        {
            if (TotalCount == 1)
            {
                return _localizer?["Pagination.WarehousesSingular"] ?? "1 warehouse";
            }

            var format = _localizer?["Pagination.WarehousesPlural"] ?? "{0} warehouses";
