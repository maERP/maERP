...
/// <summary>
/// Model for tenant list page using MVUX pattern.
/// Supports searching, sorting, and pagination.
/// </summary>
public partial record TenantListModel
{
    ...
    public IListFeed<TenantListDto> Tenants => Feed
        ...
        .SelectAsync(async (combined, ct) =>
        {
            ...
            await Pagination.UpdateAsync(_ => new TenantPaginationInfo(
    ...
    /// <summary>
    /// Navigate to create new tenant page.
    /// </summary>
    public async Task CreateTenant()
    {
        await _navigator.NavigateRouteAsync(this, Routes.TenantCreate);
    }
...
/// <summary>
/// Holds pagination state information for tenants.
/// </summary>
public record TenantPaginationInfo
{
    private readonly IStringLocalizer? _localizer;
