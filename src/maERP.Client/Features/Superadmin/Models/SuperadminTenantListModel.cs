...
public partial record SuperadminTenantListModel
{
    ...
    public IListFeed<TenantListDto> Tenants => Feed
        ...
        .SelectAsync(async (combined, ct) =>
        {
            ...
            await Pagination.UpdateAsync(_ => new SuperadminTenantPaginationInfo(
    ...
    {
        await _navigator.NavigateDataAsync(this, new SuperadminTenantEditData(tenant.Id));
    }
}

/// <summary>
/// Holds pagination state information for superadmin tenants.
/// </summary>
public record SuperadminTenantPaginationInfo
{
    private readonly IStringLocalizer? _localizer;
    ...
    /// <summary>
    /// Display text for total count info (e.g., "25 tenants").
    /// </summary>
    public string CountInfo
    {
        get
        {
            if (TotalCount == 1)
            {
                return _localizer?["Pagination.TenantsSingular"] ?? "1 tenant";
            }

            var format = _localizer?["Pagination.TenantsPlural"] ?? "{0} tenants";
